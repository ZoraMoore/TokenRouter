...
func (s *emailBindUserRepoStub) CreateWithNormalizedEmailGuard(ctx context.Context, user *service.User, _ string) error {
...
func (s *emailBindUserRepoStub) ExistsByNormalizedEmail(_ context.Context, normalizedEmail string) (bool, error) {
	...
	for email := range s.usersByEmail {
		if service.NormalizeRegistrationEmailAddress(email) == normalizedEmail {
			return true, nil
		}
	}
	return false, nil
