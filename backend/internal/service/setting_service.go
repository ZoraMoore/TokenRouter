...
func (s *SettingService) effectiveEmailOAuthConfig(settings map[string]string, provider string) config.EmailOAuthProviderConfig {
	...
	enabledKey, clientIDKey, clientSecretKey, redirectURLKey, frontendRedirectURLKey := emailOAuthSettingKeys(provider)
	if enabledKey == "" {
		return base
	}
...
// GetEmailOAuthProviderConfig 返回 GitHub/Google 邮箱 OAuth 的最终生效配置。
func (s *SettingService) GetEmailOAuthProviderConfig(ctx context.Context, provider string) (config.EmailOAuthProviderConfig, error) {
	...
	if provider == "github" && strings.TrimSpace(effective.EmailsURL) == "" {
		return config.EmailOAuthProviderConfig{}, infraerrors.InternalServer("OAUTH_CONFIG_INVALID", "oauth emails url not configured")
	}
	if strings.TrimSpace(effective.RedirectURL) == "" {
